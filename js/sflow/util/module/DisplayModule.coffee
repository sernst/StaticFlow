# sflow.util.module.DisplayModule.coffee
...
# Creates a snapshot if the module currently allows it.
    _createSnapshot: (force) =>
        if not SFLOW.mod.history
            return

        if @_allowSnapshots or force
            @_allowSnapshots = false
            SFLOW.mod.history.createSnapshot()
...
#___________________________________________________________________________________________________ _addActiveRequests
    _addActiveRequests: () =>
        ...
        if @_modulesToHide == 'ALL'
            ...
            for mid,mod of SFLOW.mod
                ...
                skip = @_indexOfItemInArray(mid, @_modulesToShow) != -1 or
                       @_indexOfItemInArray(mid, if SFLOW.mod.page then SFLOW.mod.page.focalModuleIDs() else null) != -1
        ...
        for mid in mods
            ...
            try
                if SFLOW.mod[mid] and Types.isFunction(SFLOW.mod[mid].addHideRequest)
                    SFLOW.mod[mid].addHideRequest(@_moduleID, true)
            catch err
