...
#                                signature is: callback(success, installedMajor, installedMinor)
    @checkFlashVersion: (major, minor) ->
        major = if Types.isSet(major) then major else SFLOW.CONFIG.FLASH_MAJOR
        minor = if Types.isSet(minor) then minor else SFLOW.CONFIG.FLASH_MINOR
