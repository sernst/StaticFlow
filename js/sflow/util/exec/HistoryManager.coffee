# sflow.util.exec.HistoryManager.coffee
...
# pound sign).
    createSnapshot: () =>
        ...
        if currentHistoryLength == history.length and not @_compareHashes()
            SFLOW.trace('Failed to register cache in browser history.')
            @_repeatTimer = new DataTimer(250, 0, [hashID, hashLocation], null, @_handleHashRetry)
            @_repeatTimer.start()
        else
            SFLOW.trace('Cache snapshot: ' + @_getFullHash(), snapshot)
    ...
    _cleanseLoginProfileStates: () =>
        for cacheID, cacheObj of @_cache
            for mid, snapshot of @_cache
                if SFLOW.mod[mid] and Types.isFunction(SFLOW.mod[mid].cleanseSnapshot)
                    @_cache[mid] = SFLOW.mod[mid].cleanseSnapshot(snapshot)
...
# @param {String} hash     Hash ID for the snapshot to load.
    _loadSnapshot: (hashID) =>
        SFLOW.trace("Changing hash: [#{@_hashID} -> #{hashID}]", @_cache[hashID])
...
#___________________________________________________________________________________________________ _handleHashRetry
    _handleHashRetry: () =>
        ...
        SFLOW.trace('Repeat hash history assignment.')
        ...
        if currentHistoryLength < history.length or @_compareHashes(data[0], data[1])
            @_repeatTimer.stop()
            @_repeatTimer = null
            SFLOW.trace('Hash register attempt success.')
