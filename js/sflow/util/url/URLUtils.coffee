# sflow.util.url.URLUtils.coffee
...
# Returns the data domain URL for JSONP data access.
    @getImageURL: (scriptURI) ->
        cls = URLUtils
        domain = SFLOW.CONFIG.IMAGE_DOMAIN
        if not domain
            domain = SFLOW.DOMAINS.IMAGE
...
# Returns the data domain URL for JSONP data access.
    @getDataURL: (scriptURI) ->
        cls = URLUtils
        domain = SFLOW.CONFIG.DATA_DOMAIN
        if not domain
            domain = SFLOW.DOMAINS.DATA
...
# Returns the root data domain URL for JSONP data access.
    @getJSURL: (scriptURI) ->
        cls = URLUtils
        domain = SFLOW.CONFIG.JS_DOMAIN
        if not domain
            domain = SFLOW.DOMAINS.JS
...
# Returns the root data domain URL for JSONP data access.
    @getWebAppURL: (scriptURI) ->
        cls = URLUtils
        domain = SFLOW.CONFIG.WEB_DOMAIN
        if not domain
            domain = SFLOW.DOMAINS.WEB
...
# Returns the root data domain URL for JSONP data access.
    @getCSSURL: (scriptURI) ->
        cls = URLUtils
        domain = SFLOW.CONFIG.CSS_DOMAIN
        if not domain
            domain = SFLOW.DOMAINS.CSS
...
# which can be used to manage caching on URL requests.
    @createCacheString: (forceRandom) ->
        ...
        v  = SFLOW.CONFIG.VERSION
        r  = SFLOW.CONFIG.REVISION
        cs = (if v then v else SFLOW.VERSION) + '-' + (if r then r else SFLOW.REVISION)
        if PAGE
            if PAGE.CLOUD_SIG
